...
class GWQRBasedGeoConformalSpatialPrediction:
    ...
    def avg_geoconformal_uncertainty(self, params) -> float:
        k, beta = params
        k = int(k)
        y_calib_pred = self.predict_f(self.x_calib)
        if self.nonconformity_score_f is None:
        ...
        nonconformity_scores = np.array(self.nonconformity_score_f(y_calib_pred, self.y_calib))
        results = Parallel(n_jobs=8)(delayed(self.predict_optimized_geoconformal_uncertainty_for_single_point)(self.x_test[i, :], self.coord_test[i, :], nonconformity_scores) for i in tqdm(range(N)))
        geo_uncertainty = [results[2] for results in results]
        return geo_uncertainty
    ...
    def geoconformal_uncertainty_for_single_point(self, params, *args) -> float:
        ...
        y_pred_ub = y_pred + ub
        if y_pred_lb > y_new:
            penalty = self.alpha * np.abs(y_pred_lb - y_new)
        elif y_pred_ub < y_new:
            penalty = self.alpha * np.abs(y_pred_ub - y_new)
        else:
            penalty = 0
    ...
    def optimize_prediction_interval_avg(self):
        bounds = [(1, self.x_test.shape[0]), (0.0001, self.miscoverage_level - 0.0001)]
        res = minimize(fun=self.avg_geoconformal_uncertainty, x0=(1, 0.01), bounds=bounds, method='Powell')
        return res

    def optimize_prediction_interval_for_single_point(self, x_new, coord_new, y_new):
        bounds = [(3, self.x_test.shape[0]), (0.0001, self.miscoverage_level - 0.0001)]
        res = minimize(fun=self.geoconformal_uncertainty_for_single_point, x0=(1, 0.01), bounds=bounds, method='Powell', args=(x_new, coord_new, y_new))
        return res.x

    def predict_optimized_geoconformal_uncertainty_for_single_point(self, x_new, coord, y_new, nonconformity_scores):
        k, beta = self.optimize_prediction_interval_for_single_point(x_new, coord, y_new)
        k = int(k)
        gwqr_lb = self._fit_gwqr(self.x_calib, nonconformity_scores, self.coord_calib, coord, k, beta)
        gwqr_ub = self._fit_gwqr(self.x_calib, nonconformity_scores, self.coord_calib, coord, k,
                                 1 - self.miscoverage_level + beta)
